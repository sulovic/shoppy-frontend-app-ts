...
const ModalEdit = ({ row, setShowEditModal, fetchData }: { row: FiscalReceipt; setShowEditModal: React.Dispatch<React.SetStateAction<boolean>>; fetchData: () => void }) => {
  const [updateData, setUpdateData] = useState<FiscalReceipt>(row);
  const [showSpinner, setShowSpinner] = useState(false);
  const [showSaveModal, setShowSaveModal] = useState(false);
  const axiosPrivate = useRacuniApi();
  const { authUser } = useAuth();
  const racuniService = reklamacijeServiceBuilder<FiscalReceipt>(axiosPrivate, authUser, "reklamacije");
  ...
  const handleConfirmedSaveModal = async () => {
    setShowSpinner(true);
    try {
      const parsedRacun = FiscalReceiptSchema.parse(updateData);
      const response = await racuniService.updateResource(parsedRacun.receiptNumber, parsedRacun);
      const updatedRacun = response.data.data;
      toast.success(`Račun  ${updatedRacun?.receiptNumber} - ${updatedRacun?.nameSurname} je uspešno sačuvan!`, {
        position: "top-center",
      });
      setShowEditModal(false);
      fetchData();
