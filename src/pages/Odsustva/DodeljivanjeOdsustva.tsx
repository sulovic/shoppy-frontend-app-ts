...
const DodeljivanjeOdsustva: React.FC = () => {
  ...
  const fetchData = async () => {
    setShowSpinner(true);
    try {
      const response = await axiosPrivate.get(`odsustva/dodeljena?user=${filter?.user}&godina=${filter?.godina}&vrstaOdsustva=${filter?.vrstaOdsustva}&sortBy=godina`);
  ...
  const handleDeleteOK = async () => {
    setShowSpinner(true);
    try {
      await axiosPrivate.delete(`odsustva/dodeljena/${selectedOdsustvo?.id}`);
      toast.success(`Dodeljeno odsustvo ${selectedOdsustvo?.user} - ${selectedOdsustvo?.vrstaOdsustva} je uspešno obrisano!`, { position: toast.POSITION.TOP_CENTER });
    } catch (error: any) {
      toast.error(`UPS!!! Došlo je do greške: ${error} `, { position: toast.POSITION.TOP_CENTER });
    } finally {
      setShowModal(false);
    ...
    <>
      ...
      <div className="grid grid-cols-1 justify-end gap-4 lg:flex">
        ...
        <div className="flex justify-end gap-4">
          ...
          <select id="vrstaOdsustva" aria-label="Odaberi vrstu odsustva" required value={filter?.vrstaOdsustva} onChange={handleChangeFilter}>
            <option value="*">Sve vrste odsustva</option>
            <option value="GODISNJI_ODMOR">Godišnji odmor</option>
            <option value="PLACENO_ODSUSTVO">Plaćeno odsustvo</option>
          </select>
        </div>
        <button type="button" className="button button-sky " aria-label="Dodeljena odsustva" onClick={() => navigate("/odsustva/novo-dodeljivanje")}>
          Dodeli novo odsustvo
        </button>
      </div>
      {tableData?.length ? (
        <div>
            ...
            <div className="table-responsive p-3">
              <table className="w-full text-left text-sm text-zinc-500 rtl:text-right dark:text-zinc-400 ">
                <thead className="bg-zinc-200 uppercase text-zinc-600 dark:bg-zinc-700 dark:text-zinc-400">
                  <tr>
                    ...
                    <th className="px-6 py-3">Zaposleni</th>
                    <th className="px-6 py-3">Vrsta Odsustva</th>
                    <th className="px-6 py-3">Godina</th>
                    <th className="px-6 py-3">Broj dana</th>
                    <th className="px-6 py-3">Dodelio</th>
              ...
              {showModal && (
                <Modal
                  ...
                  title="Potvrda brisanja dodeljenog odsustva"
                  question={`Da li ste sigurni da želite da obrišete dodeljeno odsustvo: ${selectedOdsustvo?.user} - ${selectedOdsustvo?.vrstaOdsustva}?`}
                />
              )}
              {updateData && showModalEdit && <ModalEditDodeljivanjeOdsustva setShowModalEdit={setShowModalEdit} updateData={updateData} setUpdateData={setUpdateData} fetchData={fetchData} />}
            </div>
          </div>
        </div>
      ) : (
        !showSpinner && <div className="p-3">Nemate podataka o dodeljenim odsustvima...</div>
      )}
      {showSpinner && <Spinner />}
    </>
  );
