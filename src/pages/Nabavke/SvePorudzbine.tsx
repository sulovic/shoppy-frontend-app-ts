...
const SvePorudzbine = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axiosPrivate.get(`nabavke/porudzbine?sortBy=id&sortOrder=desc`);
  ...
  return (
    <>
      <h3 className="my-4 ">Pregled svih porudžbina</h3>
      {tableData ? (
        <div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-8">
                ...
                <div className="col-span-2 grid grid-cols-1 content-end items-end gap-2 sm:col-span-4 sm:grid-cols-2">
                  ...
                  <div className="flex justify-end gap-2 sm:col-span-2">
                    ...
                    <button type="button" className="button button-red" aria-label="Obriši" onClick={() => handleDelete(row)} disabled={!authUser?.superAdmin}>
                      Obriši
                    </button>
                  </div>
                </div>
          ...
          {showDeleteModal && <Modal onOK={handleDeleteOK} onCancel={handleDeleteCancel} title="Potvrda brisanja porudžbine" question={`Da li ste sigurni da želite da obrišete porudžbinu: ${selectedRowDelete?.proFaktura}?`} />}
      ...
      {showHandleFiles && <HandleFiles url="nabavke/porudzbine" id={selectedRowFiles?.id} data={selectedRowFiles} fetchData={fetchData} setShowHandleFiles={setShowHandleFiles} />}
